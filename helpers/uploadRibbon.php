<?php
	...
	$conn = $DB->getConn();
	$id = $_POST['Id'];
	$target_path = "../uploads/".$id."/ribbon/";
	$date = date('Y-m-d-H-i-s');
	$dates = date('Y-m-d H:i:s');
	$target_path = $target_path . basename($date);
	$albumId;

	$find_album = "SELECT Id FROM albums WHERE UserId = ? AND Name = 'Ribbon'";
	...
	&& in_array($extension, $allowedExts)) {
		if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
			$photo_insert = "INSERT INTO photos (AlbumId,Path,Name,CreationDate) VALUES (?,?,'Ribbon',?);";
			$photo_stmt = $conn->prepare($photo_insert);
			$photo_stmt->execute(array($albumId, $target_path, $date));	

			$user_update = "UPDATE users SET RibbonPicture = ? WHERE Id = ?";
