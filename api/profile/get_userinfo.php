	...
	if ($_SERVER['REQUEST_METHOD'] === 'GET') {
		if (isset($_GET['id'])) {
			...
			for($i = 0; $i < count($locations); $i++) {
				...
				$loc_result[$i] = $loc_return[0]["Status"].": ".$loc_return[0]["Name"];
			}
			...
			for($i = 0; $i < count($educations); $i++) {
				...
				$edu_result[$i] = "Studied ".$edu_return[0]["Course"]." at ".$edu_return[0]["Name"].", from ".$edu_return[0]["StartDate"].", until ".$edu_return[0]["EndDate"];
			}
			...
			for($i = 0; $i < count($works); $i++) {
				...
				$work_return = $work_stmt->fetchAll(PDO::FETCH_ASSOC);
					$work_result[$i] = "Worked as ".$work_return[0]["Position"]." at ".$work_return[0]["Name"]." , from ".$work_return[0]["StartDate"]." , until ".$work_return[0]["EndDate"];
			}
